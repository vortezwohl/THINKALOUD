...
<div class="container">
    ...
    <div id="showUser" class="navbar">
        <h3 class="c1445" style="color: rgb(255, 79, 241);">Hey, <%= user.getAccount() %>! Got a new idea?</h3>
    </div>
</div>
<!-- Writing Area -->
<div class="write-space">
    ...
    <textarea id="title" class="input-title" placeholder="Title"><%= session.getAttribute("title") %></textarea><br/>
    <textarea id="content" class="input-content" placeholder="  Content"><%= session.getAttribute("content") %></textarea><br/>
</div>
<script>

    const saveDiary = () => {
        let diaryId = <%= session.getAttribute("diaryId") %>;
